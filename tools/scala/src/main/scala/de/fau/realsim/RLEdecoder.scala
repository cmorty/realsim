...
object RLEdecoder {
	...
	val dMap = scala.collection.mutable.Map[Int, Buffer[Int]]()
			...
			if (sp(1)(0) == 'R') {
				...
				for (ds <- data) {
					...
					if (Math.abs(cv.value - value) < lp) {
						...
						dMap.getOrElseUpdate(nid, Buffer[Int]()) += cv
		...
		for (l <- lIt) if (l != "") {
			...
			if (sp.length == 3) try {
				...
				if(linesparsed % 1000000 == 0) {
					val onow = now
					now = System.nanoTime
					println("Parsed: "  + linesparsed + " Time: " + NANOSECONDS.toMillis(now - onow))
				}
				val nid = sp(1).split(':')(2).tail.tail.hToI
